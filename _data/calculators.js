...
module.exports = {
    mortgage: {
        ...
        metaDescriptions: {
            en: "Free Mortgage Calculator 2026 — See your true monthly payment instantly. Includes taxes, insurance & PMI. No signup. Updated daily.",
        ...
        config: {
            ...
            breakdown: [
                ...
                { label: { en: "PMI", fi: "PMI", de: "PMI", es: "PMI", fr: "PMI", pt: "PMI", it: "PMI", nl: "PMI", pl: "PMI", sv: "PMI", no: "PMI", da: "PMI" }, formula: "down < principal * 0.2 ? (principal - down) * 0.005 / 12 : 0", format: "currency" },
    ...
    crypto: {
        ...
        config: {
            ...
            notices: [
                ...
                { message: { en: "⚠️ Warning: You are at a loss of ${Math.abs(result).toLocaleString()}. See 'Recovery Pump' metric for details.", es: "⚠️ Cuidado: Estás perdiendo ${Math.abs(result).toLocaleString()}. Revisa 'Subida Necesaria'.", de: "⚠️ Warnung: Sie sind ${Math.abs(result).toLocaleString()} im Minus. Details siehe 'Nötiger Anstieg'.", fr: "⚠️ Attention : Vous perdez ${Math.abs(result).toLocaleString()}. Voir 'Hausse Nécessaire'.", pt: "⚠️ Atenção: Você está com prejuízo de ${Math.abs(result).toLocaleString()}. Veja detalhes em 'Subida Necessária'.", it: "⚠️ Attenzione: Sei in perdita di ${Math.abs(result).toLocaleString()}. Vedi 'Rialzo Necessario'.", nl: "⚠️ Let op: U staat op een verlies van ${Math.abs(result).toLocaleString()}. Zie 'Benodigde Stijging'.", pl: "⚠️ Ostrzeżenie: Tracisz ${Math.abs(result).toLocaleString()}. Zobacz 'Wymagany Wzrost'.", sv: "⚠️ Varning: Du ligger minus ${Math.abs(result).toLocaleString()}. Se 'Nödvändig Uppgång' för detaljer.", no: "⚠️ Advarsel: Du har et tap på ${Math.abs(result).toLocaleString()}. Se 'Nødvendig Oppgang'.", da: "⚠️ Advarsel: Du har et tab på ${Math.abs(result).toLocaleString()}. Se 'Nødvendig Stigning'.", fi: "⚠️ Varoitus: Olet tappiolla ${Math.abs(result).toLocaleString()}. Katso tiedot kohdasta 'Tarvittava nousu'." }, condition: "result < 0", type: "warning" }
            ]
        },
    },
    solar: {
